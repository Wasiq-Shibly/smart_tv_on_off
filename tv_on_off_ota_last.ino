...
void setup() {
  ...
  data = (bool(EEPROM.read(0)));
  Serial.print("eeprom value::  ");
  Serial.println(data);
  Serial.print("eeprom value end ::  ");
...
void loop() {
  ...
  if (currentMinute > last_mm)
  {
    last_mm = last_mm + 1;


    results.value = 0x40BE12ED ;
  ...
  if (24 > currentHour &&  currentHour > 6 )
  {
    Serial.println("TV ON Time ");

    if ((timer - timef) > 35 )
    {
      tv_on();
      run_app();
      Serial.println("tv on function is called");

      check_status = true;
      EEPROM.write(0, check_status);

      delay(1000);
    }
    else {
      Serial.println("tv running ");
    }
  }
  ...
  {
    Serial.println("TV Off Time ");
    if (check_status)
    {
      Serial.println("need to turn OFF TV and called the function ");
      tv_off();
      check_status = false;
      EEPROM.write(0, check_status);
      delay(1000);
